<template>
    <article class="card p-4 h-full flex flex-col">
        <header class="mb-4 flex items-center justify-between">
            <h2 class="h1 mb-0 text-lg">{{ check.label }}</h2>
            <check-result :check="check"></check-result>
        </header>
        <div class="flex-1 flex flex-col py-4 border-b border-t border-grey-lighter min-h-32">
            <div class="flex-1 mb-4"><slot name="body" v-bind="check"></slot></div>
            <div v-if="check.enabled"><slot name="link" v-bind="check"></slot></div>
        </div>
